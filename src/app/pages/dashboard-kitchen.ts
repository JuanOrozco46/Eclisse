    ...
    <div class="relative flex flex-col gap-12 animate-in fade-in slide-in-from-right-8 duration-700">
      ...
      <header class="flex flex-col md:flex-row justify-between items-start md:items-center bg-surface-container-low border-b-2 border-outline-variant p-10 relative overflow-hidden gap-6">
        ...
        <div class="flex items-center gap-10 relative z-10">
          <div class="writing-vertical font-anton text-stark-gray opacity-20 text-[10px] tracking-brutal uppercase">SYSTEM_KDS // v3.0</div>
          <div>
        ...
        <div class="flex gap-6 flex-wrap items-center relative z-10">
          ...
          <div class="flex flex-col items-center border-2 border-tomato px-6 py-3 bg-tomato/5" *ngIf="priorityOrdersCount() > 0">
      ...
      <main class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        <!-- Empty State -->
        <div *ngIf="kitchenOrders().length === 0" class="col-span-full flex flex-col items-center justify-center py-40 border border-outline-variant border-dashed">
        ...
        <div *ngFor="let order of kitchenOrders()" 
          ...
          <div class="absolute top-3 left-3 z-20 flex gap-2">
            <span *ngIf="order.source === 'RAPPI'" 
                  class="font-label-caps text-[8px] bg-[#FF6900] text-white px-2 py-1 tracking-widest uppercase">
              RAPPI
            </span>
            <span *ngIf="order.source === 'WHATSAPP'" 
                  class="font-label-caps text-[8px] bg-emerald text-black px-2 py-1 tracking-widest uppercase">
              WHATSAPP
            </span>
            <span *ngIf="!order.source || order.source === 'POS'" 
                  class="font-label-caps text-[8px] bg-outline-variant text-stark-gray px-2 py-1 tracking-widest uppercase">
              LOCAL
            </span>
          </div>
          ...
          <div class="p-6 pt-10 border-b border-outline-variant flex justify-between items-start" 
            ...
            <div class="flex flex-col">
              ...
              <span *ngIf="order.customerPhone && order.source === 'WHATSAPP'" 
                    class="font-label-caps text-[9px] text-emerald tracking-widest mt-1">
                📱 {{order.customerPhone}}
              </span>
              <!-- Delivery status badge for WA orders -->
            ...
            <div class="flex flex-col items-end">
              <span class="font-anton text-2xl" [ngClass]="order.priority ? 'text-tomato animate-pulse' : 'text-stark-gray'">{{getElapsedTime(order)}}'</span>
          ...
          <div class="flex-1 p-6 space-y-4">
            <div *ngFor="let item of order.items" class="flex flex-col border-b border-outline-variant/30 pb-4 last:border-0">
               <div class="flex items-start justify-between">
                 <span class="font-anton text-xl text-white uppercase group-hover:text-secondary transition-colors">{{item.quantity}}x {{item.name}}</span>
               </div>
               <span *ngIf="item.note" class="font-label-caps text-[10px] text-secondary italic tracking-widest mt-1 bg-secondary/5 px-2 py-1">"{{item.note}}"</span>
            </div>
          </div>
          ...
          <div class="w-full flex flex-col">
            ...
            <!-- ── RAPPI ORDERS: simple accept flow ── -->
            <ng-container *ngIf="order.source === 'RAPPI'">
              <button *ngIf="order.status === 'PENDING'" (click)="updateStatus(order, 'PREPARING')"
            ...
            <ng-container *ngIf="order.source === 'WHATSAPP'">
              ...
              <!-- When preparing: show "Request Delivery" if not yet requested -->
              <ng-container *ngIf="order.status === 'PREPARING'">
                ...
                <!-- When delivery requested: show "Dispatch" button -->
            ...
            <!-- ── POS / LOCAL ORDERS: standard flow ── -->
          ...
          <!-- Priority Indicator -->
          <div *ngIf="order.priority" class="absolute -top-3 -right-3 bg-tomato text-white px-3 py-1 font-anton text-xs z-20">URGENTE</div>
        </div>
      </main>
    </div>
  `,
...
export class DashboardKitchen {
  ...
  getElapsedTime(order: SalesOrder): number {
    const now = Date.now();
    return Math.floor((now - order.timestamp) / 60000);
  }
}
